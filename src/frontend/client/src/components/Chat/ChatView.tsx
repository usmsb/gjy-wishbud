...
import { recordAuditLogApi } from '~/api';
...
const QuickAccess = () => {
  ...
  useEffect(() => {
    const loadQuickActions = async () => {
      try {
        const response = await fetch(`${__APP_ENV__.BASE_URL}/quick-actions.json`);
        const data = await response.json();
        setQuickActions(data.actions || data);
      } catch (error) {
  ...
  async function recordAuditLog(auditData) {
    try {
      const result = await recordAuditLogApi(auditData);
      if (result.code === 0) {
        console.log('审计日志记录成功');
      } else {
        console.error('审计日志记录失败:', result.message);
      }
    } catch (error) {
      console.error('审计日志记录出错:', error);
      // 忽略错误，不影响主要功能
    }
  }
  ...
  const handleQuickAction = (action) => {
    ...
    if(action.type===10|| action.type === 5)
      ...
      const uri= action.id+'?username='+encodedUsername;
      ...
      recordAuditLog({
        ...
        object_type: "dify_url",
        object_id: action.id,
        object_name: action.name,
        note: `用户点击了${action.name}按钮，打开链接: ${uri}`
      });
    }
  };
