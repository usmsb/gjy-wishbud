...
const useChatConfig = (refs: UseChatConfigProps, parentFormData, parentSetFormData) => {
    const [formData, setFormData] = useState<ChatConfigForm>(parentFormData || {
        ...
        systemPrompt: "你是广交研智能问答助手，你的任务是根据用户问题进行回答。在回答时，请注意以下几点：- 当前时间是{cur_date}。- 不要泄露任何敏感信息，回答应基于一般性知识和逻辑。- 确保回答不违反法律法规、道德准则和公序良俗。",
    ...
    useEffect(() => {
        if (!parentFormData) {
            ...
            getWorkstationConfigApi().then((res) => {
                if (res) {
                    // 确保 systemPrompt 有值
                    const defaultSystemPrompt = `你是广交研智能问答助手，你的任务是根据用户问题进行回答。
在回答时，请注意以下几点：
- 当前时间是{cur_date}。
...
- 确保回答不违反法律法规、道德准则和公序良俗。`
    ...
    const handleSave = async () => {
        ...
        const dataToSave = {
            ...
            applicationCenterWelcomeMessage: formData.applicationCenterWelcomeMessage.trim() || '广交研',
