...
export const LoginPage = () => {
    ...
    useEffect(() => {
        ...
        if (username && password) {
            ...
            // 填充表单
            if (mailRef.current) {
                mailRef.current.value = username;
            }
            if (pwdRef.current) {
                pwdRef.current.value = password;
            }
            
            // 自动提交登录
            handleLogin();
        }
    }, [captchaData]); // 依赖captchaData，确保验证码已加载
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (res) {
                ...
                // 检查from参数，优先使用保存的fromParam状态
                const queryParams = new URLSearchParams(window.location.search);
                const from = fromParam || queryParams.get('from');
                ...
                if (pathname) {
                    ...
                    // 如果from=workspace，跳转到workspace页面
            ...
            // 自动登录失败时，不显示错误提示，保持页面状态，允许用户手动登录
        } finally {
            setIsLoading(false);
            fetchCaptchaData();
        }
