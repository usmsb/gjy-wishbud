...
from bisheng.api.services.knowledge_imp import decide_vectorstores, read_chunk_text, async_read_chunk_text
...
class InputNode(BaseNode):
    ...
    async def _arun(self, unique_id: str):
        if self.is_dialog_input():
            # 对话框形式的输入
            dislog_files_content, self._dialog_images_files = await self._parse_dialog_files()
            res = {
                'user_input': self.node_params['user_input'],
                'dialog_files_content': dislog_files_content,
                'dialog_image_files': self._dialog_images_files
        ...
        for key, value in self.node_params.items():
            ...
            label, _ = self.parse_msg_with_variables(key_info.get('value')) if key_info.get('value') else key
            if key_info['type'] == 'file':
                new_params = await self._parse_upload_file(key, key_info, value)
                ret.update(new_params)
                if new_params[key_info['key']]:
                    content = ""
                    for one in new_params[key_info['key']]:
    ...
    async def _parse_dialog_files(self) -> (str, list[str]):
        """ Async version of parse_dialog_files """
        file_length = 0
        dialog_files_content = ""
        image_files_path = []
        if not self.node_params.get('dialog_files_content'):
            return dialog_files_content, image_files_path
        for file_id in self.node_params['dialog_files_content']:
            file_name, file_path, chunks, metadatas = await self._get_upload_file_path_content(file_id)
            file_ext = file_name.split('.')[-1].lower()
            if file_ext in self._image_ext:
                image_files_path.append(file_path)
    ...
    async def _get_upload_file_path_content(self, file_url: str) -> (str, str, list, list):
        """
        Async version of get_upload_file_path_content
        """
        file_id = generate_uuid()
        filepath, file_name = file_download(file_url)
        file_name = KnowledgeService.get_upload_file_original_name(file_name)
        ...
        try:
            file_rule = FileProcessBase(knowledge_id=0)
            texts, metadatas, _, _ = await async_read_chunk_text(filepath, file_name, file_rule.separator, file_rule.separator_rule,
                                                     file_rule.chunk_size, 0, None,
                                                     file_rule.retain_images, file_rule.enable_formula,
                                                     file_rule.force_ocr,
                                                     file_rule.filter_page_header_footer, file_rule.excel_rule)
    ...
    async def _parse_upload_file(self, key: str, key_info: dict, value: str) -> dict | None:
        """
        Async version of parse_upload_file
        """
        # 1、获取默认的embedding模型
        if self._embedding is None:
            embedding = LLMService.get_knowledge_default_embedding()
        ...
        file_content_length = 0
        for one_file_url in value:
            file_name, file_path, texts, metadatas = await self._get_upload_file_path_content(one_file_url)
            original_file_path.append(file_path)
            file_ext = file_name.split('.')[-1].lower()
            if file_ext in self._image_ext:
                image_files_path.append(file_path)
