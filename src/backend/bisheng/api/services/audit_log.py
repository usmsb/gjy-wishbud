...
class AuditLogService:
    ...
    @classmethod
    def create_audit_log(cls, user: UserPayload, ip_address: str, system_id: str, event_type: str,
                        object_type: str = "none", object_id: str = "", object_name: str = "", note: str = ""):
        """
        通用创建审计日志的方法
        """
        logger.info(f"act=create_audit_log user={user.user_name} ip={ip_address} system_id={system_id} event_type={event_type}")
        # 获取用户所属的分组
        user_group = UserGroupDao.get_user_group(user.user_id)
