...
async def async_process_file_task(
        knowledge: Knowledge,
        db_files: List[KnowledgeFile],
        separator: List[str],
        separator_rule: List[str],
    ...
    try:
        ...
        for i, db_file in enumerate(db_files):
            ...
            if preview_cache_keys and i < len(preview_cache_keys):
                preview_cache_key = preview_cache_keys[i]
            
            # 为每个文件创建独立的向量客户端，避免连接冲突
        ...
        logger.exception("async_process_file_task error")
        new_files = KnowledgeFileDao.select_list([file.id for file in db_files])
        new_files_map = {file.id: file for file in new_files}
        for file in db_files:
            if new_files_map[file.id].status == KnowledgeFileStatus.PROCESSING.value:
...
async def async_process_single_file(
    ...
    """异步处理单个知识文件"""
    try:
        logger.info(f"async_process_single_file_begin file_id={db_file.id} file_name={db_file.file_name}")
        
        # 为每个文件创建独立的embedding和向量客户端
        ...
        # 处理单个文件
        await async_add_file_embedding(
            vector_client,
            es_client,
            minio_client,
            db_file,
        ...
        logger.exception(f"async_process_single_file_fail file_id={db_file.id} file_name={db_file.file_name}")
        db_file.status = KnowledgeFileStatus.FAILED.value
        db_file.remark = str(e)[:500]
    finally:
        logger.info(f"async_process_single_file_end file_id={db_file.id} file_name={db_file.file_name}")
...
async def async_add_file_embedding(
        vector_client,
        es_client,
        minio_client,
        db_file: KnowledgeFile,
    ...
    # download original file
    logger.info(
        f"async start download original file={db_file.id} file_name={db_file.file_name}"
    )

    file_url = minio_client.get_share_link(db_file.object_name)
    filepath, _ = await asyncio.to_thread(file_download, file_url)
    ...
    texts, metadatas, parse_type, partitions = await asyncio.to_thread(
        read_chunk_text,
        filepath,
        db_file.file_name,
        separator,
    ...
    logger.info(
        f"async chunk_split file={db_file.id} file_name={db_file.file_name} size={len(texts)}"
    )
    for metadata in metadatas:
        metadata.update(
            {
    ...
    logger.info(f"async add_vectordb file={db_file.id} file_name={db_file.file_name}")
    # 异步存入milvus
    try:
        if hasattr(vector_client, 'aadd_texts'):
            await vector_client.aadd_texts(texts=texts, metadatas=metadatas)
        else:
            await asyncio.to_thread(vector_client.add_texts, texts=texts, metadatas=metadatas)
    except Exception as e:
        logger.exception(f"async add_vectordb failed: {e}")
        raise

    logger.info(f"async add_es file={db_file.id} file_name={db_file.file_name}")
    # 异步存入es
    try:
        if hasattr(es_client, 'aadd_texts'):
            await es_client.aadd_texts(texts=texts, metadatas=metadatas)
        else:
            await asyncio.to_thread(es_client.add_texts, texts=texts, metadatas=metadatas)
    except Exception as e:
        logger.exception(f"async add_es failed: {e}")
        raise

    logger.info(f"async add_complete file={db_file.id} file_name={db_file.file_name}")

    if preview_cache_key:
        await asyncio.to_thread(KnowledgeUtils.delete_preview_cache, preview_cache_key)
    ...
    if db_file.file_name.endswith((".doc", ".ppt", ".pptx")):
        ...
        logger.info(
            f"async upload_preview_file_to_minio file={db_file.id} tmp_object_name={tmp_preview_file}, preview_object_name={preview_object_name}"
        )
        
        if await asyncio.to_thread(
            minio_client.object_exists_sync, 
        ...
        logger.info(
            f"async upload_preview_file_over file={db_file.id} tmp_object_name={tmp_preview_file}, preview_object_name={preview_object_name}"
        )
