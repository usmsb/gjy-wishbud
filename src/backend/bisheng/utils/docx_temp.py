...
class DocxTemplateRender(object):
    ...
    def _set_cell_text_with_breaks(self, cell, text: Any):
        """Set table cell text while preserving embedded line breaks in Word."""
        paragraph = cell.paragraphs[0] if cell.paragraphs else cell.add_paragraph()
        self._set_paragraph_text_with_breaks(paragraph, text)
        ...
        try:
            ...
            for i, row_data in enumerate(table_data):
                for j, cell_data in enumerate(row_data):
                    if j < cols:
                        cell = table.cell(i, j)
                        self._set_cell_text_with_breaks(cell, cell_data)
