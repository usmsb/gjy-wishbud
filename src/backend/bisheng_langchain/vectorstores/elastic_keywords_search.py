...
class ElasticKeywordsSearch(VectorStore, ABC):
    ...
    async def aadd_texts(
        ...
        **kwargs: Any,
        ...
        """Asynchronously run more texts through the keywords and add to the vectorstore.
        ...
        # check to see if the index already exists
        try:
            await asyncio.to_thread(self.client.indices.get, index=self.index_name)
            if texts and self.drop_old:
                await asyncio.to_thread(self.client.indices.delete, index=self.index_name)
                await asyncio.to_thread(self.create_index, self.client, self.index_name, mapping)
        except NotFoundError:
        ...
        # Use bulk insertion with async wrapper
        await asyncio.to_thread(bulk, self.client, requests)
        
        if refresh_indices:
            await asyncio.to_thread(self.client.indices.refresh, index=self.index_name)
