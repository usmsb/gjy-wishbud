...
class Milvus(MilvusLangchain):
    ...
    def add_texts(
        ...
        for i in range(0, total_count, batch_size):
            ...
            # Insert into the collection with non-blocking options
            try:
                # Use async insert if available, otherwise use sync insert with timeout
    ...
    async def aadd_texts(
        ...
        """Asynchronously insert text data into Milvus.
        
        Args:
            texts (Iterable[str]): The texts to embed.
            metadatas (Optional[List[dict]]): Metadata dicts attached to each of the texts.
            timeout (Optional[int]): Timeout for each batch insert.
            batch_size (int, optional): Batch size to use for insertion.
            no_embedding (bool, optional): If True, skip embedding generation.
        ...
        if not no_embedding:
            try:
                ...
                if hasattr(self.embedding_func, 'aembed_documents'):
                    embeddings = await self.embedding_func.aembed_documents(texts)
                else:
                    # Fallback to sync embedding with async wrapper
                    embeddings = await asyncio.to_thread(self.embedding_func.embed_documents, texts)
            except NotImplementedError:
                # Fallback for embeddings that only support query
                if hasattr(self.embedding_func, 'aembed_query'):
                    embeddings = await asyncio.gather(*[self.embedding_func.aembed_query(x) for x in texts])
                else:
                    embeddings = await asyncio.gather(*[asyncio.to_thread(self.embedding_func.embed_query, x) for x in texts])
        ...
        if not isinstance(self.col, Collection):
            ...
            await asyncio.to_thread(self._init, embeddings, metadatas)
        ...
        for i in range(0, total_count, batch_size):
            ...
            try:
                # Use asyncio.to_thread to run insert in separate thread
                res = await asyncio.to_thread(self.col.insert, insert_list, timeout=timeout, **kwargs)
                pks.extend(res.primary_keys)
            except ConnectionNotExistException as e:
                logger.warning(f'retrying connection to milvus {e}')
                # reconnect to milvus
                await asyncio.to_thread(self._create_connection_alias, self.connection_args, self.alias)
